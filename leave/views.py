    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if leave_request.start_date < datetime.now().date():
                ...
                if not (user.is_superuser or user.is_manager or user == leave_request.employee_id.employee_work_info.reporting_manager_id):
                    return HttpResponse(_("You do not have permission to create a backdated leave request."), status=403)
