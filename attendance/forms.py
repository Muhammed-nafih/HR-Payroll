...
class AttendanceRequestForm(ModelForm):
    ...
    def as_p(self, *args, **kwargs):
        """
        Render the form fields as HTML with Bootstrap styling.
        """
        context = {"form": self}
    ...
    def save(self, commit=True):
        """
        Override the save method to handle cases where
        'employee_overtime' might be missing.
        """
        instance = super().save(commit=False)
        try:
            # If the instance doesn't have a valid value for employee_overtime,
            # set a default (e.g., 0)
            if getattr(instance, "employee_overtime", None) is None:
                instance.employee_overtime = 0
        except AttributeError:
            # If the attribute doesn't exist at all, you might want to log this
            # or pass if it's not required.
            pass
        if commit:
            instance.save()
        return instance
